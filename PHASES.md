...
Статус:           ✅ fix/shop-battle-improvements завершён (6 исправлений)
                  ...
                  ✅ polish: PNG-фигуры, цвета доски, перф-оптимизации,
                     система эффектов
Последнее обновление: 2026-06-14
Ожидает апрува:   НЕТ
```

## polish: PNG-фигуры, цвета доски, перф-оптимизации, система эффектов (2026-06-14)

- [x] PNG-фигуры (Cburnett, перекрашены) в `ChessPiece.tsx` вместо
      `SvgXml` — `Image` + `fadeDuration={0}` + `React.memo` с
      кастомным компаратором (`pieceKey`/`size`)
- [x] Цвета клеток доски: светлая `#d4e0f9`→`#b8bbf7`, тёмная
      `#8973f6`→`#7a66f4` (`RadialGradient` через `react-native-svg`,
      временно заменён на плоский цвет для перф-теста — см.
      HANDOVER.md Known issues)
- [x] Перф-оптимизации: обновление стейта после хода игрока/ИИ
      отложено через `requestAnimationFrame` в `chaos-battle.tsx`
- [x] ELO противника в шапке боя и кнопка возврата (`‹`) на экране боя
...
[x] MVP ПРИНЯТ
Дата: 2026-06-04
Комментарий: Все 5 фаз завершены, тег v1.0.0-mvp выставлен
```
...
| 5    | 2026-06-04     | 2026-06-04  | MVP принят, тег v1.0.0-mvp |
